  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white/70 border border-slate-200/80 p-6 rounded-2xl shadow-sm backdrop-blur-sm">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 tracking-tight flex items-center">
            <UserCog className="mr-3 h-6 w-6 text-blue-600" />
            Gestão de Usuários
          </h2>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Administradores', count: users.filter(u => u.tipo_usuario === 'admin').length, color: 'text-red-600', bg: 'bg-red-50 border-red-100' },
          { label: 'Médicos / Cuidadores', count: users.filter(u => u.tipo_usuario === 'medico').length, color: 'text-blue-600', bg: 'bg-blue-50 border-blue-100' },
          { label: 'Familiares', count: users.filter(u => u.tipo_usuario === 'familiar').length, color: 'text-slate-600', bg: 'bg-slate-50 border-slate-100' },
        ].map((stat) => (
          <div key={stat.label} className={`rounded-xl border p-4 ${stat.bg} text-center`}>
                  ...
                  <tr key={u.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-600 flex-shrink-0">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold border ${
                        u.tipo_usuario === 'admin' ? 'bg-red-50 text-red-700 border-red-200' :
                        u.tipo_usuario === 'medico' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-2 rounded-lg border border-slate-100 bg-red-50 hover:bg-white hover:text-red-600 hover:border-red-100 transition-colors duration-200 cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg shadow-2xl p-6 relative animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              />
              <div>
                <label htmlFor="user-tipo" className="block text-sm font-semibold text-slate-700 tracking-wide mb-1.5">
                ...
                <select
                  ...
                  className="block w-full rounded-xl border border-slate-200 bg-white/70 px-4 py-3 text-sm font-medium text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all duration-200"
                >
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
                <Button onClick={closeModal} variant="outline" className="cursor-pointer">
                  Cancelar
                </Button>
                <Button
                  type="submit"
      ...
      {showDeleteModal && deletingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md shadow-2xl p-6 relative animate-scale-up">
            <button onClick={closeModal} className="absolute top-5 right-5 text-slate-400 hover:text-slate-700 p-1.5 rounded-lg hover:bg-slate-50 cursor-pointer">
              <X className="h-5 w-5" />
            </button>
            <div className="text-center">
              ...
              <p className="text-sm text-slate-500 mt-2">
                Você está prestes a excluir <span className="font-bold text-slate-800">{deletingUser.nome}</span>. Esta ação não pode ser desfeita.
              </p>
            </div>
